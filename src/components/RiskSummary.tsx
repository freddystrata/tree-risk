          ...
          {(() => {
            ...
            return RISK_LEVELS.map(level => {
              ...
              const segmentElement = (
                <div
                  key={level.name}
                  className={`absolute h-full ${level.color} flex items-center justify-center transition-all duration-200`}
                  style={{
                    left: `${currentPercent}%`,
                    width: `${percentage}%`,
                  }}
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs">
          {RISK_LEVELS.map(level => {
            ...
            return count > 0 ? (
              <div
                key={level.name}
                className={`px-2 py-1 rounded ${level.color} ${level.textColor || 'text-gray-800'}`}
              >
                {level.name}: {count} ({percentage.toFixed(1)}%)
              </div>
            ) : null;
          })}
        </div>
